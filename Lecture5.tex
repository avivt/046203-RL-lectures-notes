...
\begin{itemize}
  ...
  \item Using vector notation, it may be seen that
          \[{V_n}{\kern 1pt}  = {r^\pi } + {P^\pi }{r^\pi } +  \ldots  + {({P^\pi })^{n - 1}}{r^\pi } + {({P^\pi })^n}{V_0} = \sum\limits_{t = 0}^{n - 1} {{{({P^\pi })}^t}{r^\pi }}  + {({P^\pi })^n}{V_0}.\]
Similarly,    ${V^\pi } = \sum\limits_{t = 0}^\infty  {{{({P^\pi })}^t}{r^\pi }}$.
\end{itemize}
...
Similarly, define the discounted-return \textbf{Dynamic Programming Operator}  $T^*:\mathbb R^{|S|} \to \mathbb R^{|S|}$ as follows: For any $V=(V(s))\in R^{|S|}$,
...
\[T_{}^*({V_2})(s) \ge r(s,\bar a) + \gamma \sum\nolimits_{s' \in S} {p(s'|s,\bar a)V(s')} \]
...
\[\pi (s) \in \arg {\max _{a \in A}}\left\{ {r(s,a) + \gamma \sum\nolimits_{s' \in S} {p(s'|s,a){V_n}(s')} } \right\},\]
